  ...
  async def button_callback1(self, button, interaction):    
    if interaction.user.id == 358594990982561792 or interaction.user.id == 253645051702411264:
      ...
      if (Request.status_code == 200):
        ...
        await interaction.message.reply(embed=discord.Embed(description=f"*Unknown error has occurred*", color=0xED4245))
        
        async with interaction.channel.typing():
        
          msga1 = await interaction.channel.send("Checking for 2fa...")
          
          ## 1 ## Get challengeId
          challengeID = session.post('https://economy.roblox.com/v2/spend-friction/two-step-verification/generate').json()

          ## 2 ## Send Code
          data2 = {"challengeId":challengeID, "actionType":"ItemTrade"}
          session.post(f'https://twostepverification.roblox.com/v1/users/{userID}/challenges/email/send-code', data=data2).json()

          ## 3 ## Get Code
          #credentials
          username ="serverholder6@gmail.com"
          #generated app password
          app_password= "mllmahwbhaymmpbt"
          ...
          #select inbox
          mail.select("INBOX")
          result, data = mail.search(None, '(FROM "accounts@roblox.com")')
          ...
          for part in email_message.walk():
              if part.get_content_type()=="text/plain" or part.get_content_type()=="text/html":
                  ...
                  #print("Message: \n", message.decode())
                  #mag = message.split("2-Step Verification Code: ")[1].split("\r")[0].split("</h4></td>")[0].splitlines()
                  try:
                      mag = message.split("\r\n\r")[1].split("\n")[1].split("@media (prefers-color-scheme: dark) {")[0].split()
                      if mag[0] == "2-Step":
                          code = mag[3]
                  except IndexError:
                      pass

          ## 4 ## Get verificationToken
          data4 = {"challengeId":challengeID, "actionType":"ItemTrade", "code":code}
          Token = session.post(f'https://twostepverification.roblox.com/v1/users/{userID}/challenges/email/verify', data=data4).json()["verificationToken"]

          ## 5 ## Redeem verificationToken
          data5 = {"challengeToken":challengeID, "verificationToken":Token}
          New_Request = session.post('https://economy.roblox.com/v2/spend-friction/two-step-verification/redeem', data=data5)
          
          if New_Request.status_code == 200:
            
            await msga1.delete()
            await interaction.message.reply(embed=discord.Embed(description=f"*Successfully verified with 2fa*", color=0x57F288))
            
            msga2 = await interaction.channel.send("Re-executing the cmd..")
            ...
            if (Request.status_code == 200):
              ...
              await interaction.message.reply(embed=discord.Embed(description=f"*Error occured again*", color=0xED4245))
